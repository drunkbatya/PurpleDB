...
uint32_t detect_lines_to_delete(uint16_t count, uint32_t size1, FILE *fptr,
...
void delete(char **arr)
{
    ...
    fptr = fopen(path, "rb+");
    where_col_offset = find_offset_for_column(arr[1], fptr, column_number, &var_type);
    ...
    size1 = get_size_by_datatype_simple(&var_type);
